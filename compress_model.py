...
def valid_feature_type(feature_type):
    ...
    bool2 = feature_type.startswith('favor+') and feature_type.split('_')[1].isdigit()
...
class SLiMPerformer(torch.nn.Module):
    ...
    def __init__(self, vocab_size, vocab_dim, hidden_dim, n_layers, ffn_dim, n_heads, feature_type, compute_type):
        ...
        self.layers = torch.nn.ModuleList([
            SLiMPerformerLayer(hidden_dim, ffn_dim, n_heads, feature_type, compute_type) for _ in range(n_layers)
        ])
    ...
    def _concat_pos_embs(self, x, start_index):
        ...
        positions = torch.arange(start_index, start_index + x.shape[1], dtype=x.dtype, device=x.device)
        freqs = torch.exp(torch.arange(0, pos_emb_size, 2, dtype=x.dtype, device=x.device) * (-np.log(10000) / pos_emb_size))
...
class SLiMPerformerLayer(torch.nn.Module):
    ...
    def __init__(self, hidden_dim, ffn_dim, n_heads, feature_type, compute_type):
        ...
        self.attention = MultiHeadAttention(feature_type, n_heads, hidden_dim, compute_type)
...
class MultiHeadAttention(torch.nn.Module):
    ...
    def _get_queries_keys_values(self, inputs, rfs):
        ...
        queries = queries.reshape([queries.shape[0], queries.shape[1], self._n_heads, -1])
        keys = keys.reshape([keys.shape[0], keys.shape[1], self._n_heads, -1])
        values = values.reshape([values.shape[0], values.shape[1], self._n_heads, -1])
...
def _sample_orth_matrix(size, device):
    ...
    S = torch.triu(subspace.T.mm(subspace)) - 0.5 * torch.eye(subspace.shape[1], device=device)

    result = torch.eye(subspace.shape[0], device=device) - subspace.mm(torch.inverse(S)).mm(subspace.T)
